...
patch(MessageModel.prototype, {
    pinned_at: Record.attr(undefined, { type: "datetime" }),
});

patch(ThreadModel.prototype, {
    pinnedMessages: Record.many("Message", {
        compute() {
            if (!this.messages) return [];
            return this.messages.filter((m) => m.pinned_at);
        },
    }),
});

messageActionsRegistry.add("pin-toggle", {
    condition: (component) => !component.props.message.is_transient && component.store.self.isInternalUser,
    ...
    async loadCustomFileView() {
        ...
        try {
            ...
            if (this.state.file.isXlsx && typeof window.XLSX === 'undefined') {
                await loadJS("/rich_text_chatter_18/static/lib/xlsx.full.js");
            } else if (this.state.file.isDocx && typeof window.mammoth === 'undefined') {
                await loadJS("/rich_text_chatter_18/static/lib/mammoth.browser.js");
            }
